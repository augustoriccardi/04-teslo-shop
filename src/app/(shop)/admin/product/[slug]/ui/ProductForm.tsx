...
export const ProductForm = ({ product, categories, slug }: Props) => {
  ...
  const onSubmit = async (data: FormInputs) => {
    ...
    //Si el product.id viene quiero verificar que sea un uuid, de lo contrario no hago el append ni tampoco se va a validar con zod en createUpdateProduct
    if (product.id) {
    ...
    if (!ok) {
      ...
      const successMessage = `ID:${updatedProduct?.id} ${
        slug === "new" ? "creado" : "actualizado"
      } correctamente`;
              ...
              {product.ProductImage?.map((image) => (
                <div key={image.id}>
                  <ProductImage
                    alt={product.title ?? ""}
                    src={image.url}
                    width={isImageScaled ? 700 : 300}
                    height={isImageScaled ? 700 : 300}
